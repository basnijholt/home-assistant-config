...
};customElements.define("lovelace-template-card", class extends HTMLElement {
  constructor() {
    super(), this.attachShadow({ mode: "open" });
  }set hass(e) {
    this._card && (this._card.hass = e);
  }setConfig(e) {
    if (!e.template) throw new Error("Missing template object in your config");const t = g();if (!t.config && !t.config.lovelace_templates) throw new Error("The object lovelace_templates doesn't exist in your main lovelace config.");const n = t.config.lovelace_templates[e.template];if (!n) throw new Error(`The template "${e.template}" doesn't exist in lovelace_templates`);const o = this.shadowRoot;for (; o && o.hasChildNodes();) o.removeChild(o.lastChild);const r = document.createElement("div");r.id = "root", o.appendChild(r);const i = (e, t) => {
